...
export default function PMProjectControlPanel({ control, isLoading, error, onRefresh, onOpenRequirements, aiEnabled = false, explanations = {}, onExplain }) {
...
function Finding({ finding, onOpenRequirements, aiEnabled, explanation, onExplain }) {
  ...
  return <article className="rounded-md border border-border p-4">
    <p className="text-xs font-semibold uppercase tracking-wide text-text-secondary">Severity: {finding.severity}</p><h4 className="mt-1 font-semibold text-text">{finding.title}</h4><p className="mt-1 text-sm text-text-secondary">{finding.summary}</p>
    <dl className="mt-3 grid gap-2 sm:grid-cols-2">{finding.evidence.map((item) => <div key={item.key} className="min-w-0"><dt className="text-xs text-muted">{item.label}</dt><dd className="break-words text-sm text-text">{displayEvidence(item)}</dd></div>)}</dl>
    <p className="mt-3 text-sm text-text-secondary"><span className="font-medium text-text">Recommended action: </span>{finding.recommended_action}</p>
    {aiEnabled && <button type="button" disabled={explanation?.loading} onClick={() => onExplain?.(finding)} className="mt-3 inline-flex rounded-md border border-border px-3 py-2 text-sm font-medium text-text-secondary hover:bg-surface-muted disabled:cursor-not-allowed disabled:opacity-60">{explanation?.loading ? "Generating explanation…" : "Explain with AI"}</button>}
    {explanation?.error && <p className="mt-3 text-sm text-danger" role="alert">Explanation could not be loaded. You can try again; the verified finding remains available.</p>}
    {explanation?.data && <div className="mt-3 rounded-md bg-surface-muted p-3 text-sm text-text-secondary" role="status"><h5 className="font-medium text-text">{explanation.data.source === "AI" ? "AI explanation" : "System fallback explanation"}</h5><p className="mt-1 break-words">{explanation.data.explanation}</p><p className="mt-2 text-xs text-muted">Generated from the verified system finding. The underlying finding and action remain system-determined.</p></div>}
    {requirementsAction && <button type="button" onClick={onOpenRequirements} className="mt-3 inline-flex rounded-md border border-border px-3 py-2 text-sm font-medium text-text-secondary hover:bg-surface-muted">View requirements</button>}{destination && <Link to={destination.to} className="mt-3 inline-flex rounded-md border border-border px-3 py-2 text-sm font-medium text-text-secondary hover:bg-surface-muted">{destination.label}</Link>}
  </article>;
}
