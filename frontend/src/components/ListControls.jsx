export default function ListControls({ page, totalPages, total, search, onSearchChange, onPrevious, onNext }) {
  return (
    <div className="mt-4 flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-center sm:justify-between">
      {onSearchChange && <ListSearch search={search} onSearchChange={onSearchChange} />}
...
export function ListSearch({ search, onSearchChange, id = "list-search" }) {
  return (
    <>
      <label className="sr-only" htmlFor={id}>Search this list</label>
      <input id={id} type="search" value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="Search" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm sm:max-w-xs" />
    </>
  );
}
