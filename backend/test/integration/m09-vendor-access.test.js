...
test("M09 guards PM membership, scopes client data, and preserves history after revocation", async () => {
  ...
  const submitted = await request("POST", `/vendor/projects/${scopedProject.id}/requirements/${scopedProject.requirements[0].id}/candidates`, { contractor_id: contractor.data.id }, vendorA.token); assert.equal(submitted.response.status, 201);
