...
test("M06 preserves daily drafts, requires reasons, supports correction/resubmission, and atomically bulk reviews", { timeout: 60000 }, async () => {
  ...
  const project = await request("POST", "/pm/projects", { name: "M06 Daily Workflow", start_date: workDate, expected_hours: 16, requirements: [{ skill: "FRONTEND", required_count: 1 }] }, pm.token);
  ...
  const weekendPolicy = await request("PATCH", `/pm/projects/${projectId}`, { allow_weekend: true }, pm.token);
  assert.equal(weekendPolicy.response.status, 200, JSON.stringify(weekendPolicy.data));
  ...
  const corrected = await request("PATCH", `/contractor/timesheets/${draft.data.id}`, { workDate, hoursLogged: 8, description: "Build workflow with tests" }, contractor);
