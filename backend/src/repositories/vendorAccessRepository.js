...
async function listForCompany(companyId){
  const [connections]=await pool.query("SELECT r.vendor_id AS id,u.name,u.email,r.status FROM client_vendor_relationships r JOIN users u ON u.id=r.vendor_id WHERE r.client_company_id=? ORDER BY u.name",[companyId]);
  if(!connections.length)return [];
  const [grants]=await pool.query("SELECT DISTINCT pv.vendor_id,p.id AS project_id,p.name AS project_name FROM project_vendors pv JOIN projects p ON p.id=pv.project_id JOIN project_managers pm ON pm.user_id=p.pm_id WHERE pm.company_id=? AND pv.status='ACTIVE' ORDER BY p.name",[companyId]);
  const grantsByVendor=new Map();
  for(const g of grants){
    if(!grantsByVendor.has(g.vendor_id))grantsByVendor.set(g.vendor_id,[]);
    grantsByVendor.get(g.vendor_id).push({id:g.project_id,name:g.project_name});
  }
  return connections.map(c=>({...c,projects:grantsByVendor.get(c.id)||[]}));
}
