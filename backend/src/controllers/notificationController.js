...
const list=asyncHandler(async(req,res)=>{await service.materializeExpiringDocuments();const items=await service.list(req.user.userId);res.json({items,unread_count:items.filter(x=>!x.read_at).length});});
