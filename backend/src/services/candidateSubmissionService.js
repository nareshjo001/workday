...
async function submit(vendorId, p, actor) {
  ...
  try { await c.beginTransaction();
    ...
    await c.commit(); return { id, project_id:p.projectId, requirement_id:p.requirementId, contractor_id:worker.id, status:"SUBMITTED", proposed_start_date:startDate, proposed_end_date:endDate };
...
async function decide(pmId, id, status, reason, actor) {
  ...
  try { await c.beginTransaction();
    ...
    await c.commit(); return { id, status, assignment_id:assignmentId };
  } catch (e) { await c.rollback().catch(()=>{}); throw e; } finally { c.release(); }
}
