...
function filters(query = {}) { return { clientId: positive(query.clientId, 'clientId'), projectId: positive(query.projectId, 'projectId'), skillId: positive(query.skillId, 'skillId'), status: query.status ? String(query.status).toUpperCase() : null, startDate: iso(query.startDate, 'startDate'), endDate: iso(query.endDate, 'endDate') }; }
...
async function dashboard(role, userId, rawQuery) {
  ...
  const [[projects]] = await pool.query(`SELECT COUNT(DISTINCT p.id) projects, COUNT(DISTINCT pm.company_id) connected_clients, COALESCE(SUM(p.budget),0) budget, COALESCE(SUM(p.expected_hours),0) planned_hours, COALESCE(SUM(CASE WHEN p.status='ACTIVE' THEN 1 ELSE 0 END),0) active_projects FROM projects p JOIN project_managers pm ON pm.user_id=p.pm_id WHERE ${scope.where}`, scope.values);
  ...
  return { filters: f, workforce: { ...Object.fromEntries(Object.entries(staffing).map(([k,v])=>[k,Number(v)])), connected_clients:Number(projects.connected_clients) }, candidates: Object.fromEntries(Object.entries(candidates).map(([k,v])=>[k,Number(v)])), time: Object.fromEntries(Object.entries(hours).map(([k,v])=>[k,Number(v)])), compliance: { expiring_documents:Number(compliance.expiring_documents) }, financial: { ...Object.fromEntries(Object.entries(finance).map(([k,v])=>[k,Number(v)])), billable_uninvoiced_amount:Number(billable.billable_uninvoiced_amount), budget:Number(projects.budget), planned_hours:Number(projects.planned_hours), active_projects:Number(projects.active_projects), margin }, };
}
module.exports = { dashboard, filters };
